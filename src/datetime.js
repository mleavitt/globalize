define([
	...
	datetime.patternRE = /([a-z])\1*/ig;
	datetime.weekDays = [ "sun", "mon", "tue", "wed", "thu", "fri", "sat" ];
	 ...
	 */
	datetime.format = function( date, pattern, cldr ) {
		var widths = [ "abbreviated", "wide", "narrow" ];
		return pattern.replace( datetime.patternRE, function( current ) {
			var pad, ret,
			...
			switch ( chr ) {
				...
				case "D":
					// FIXME getDayOfYear
					ret = datetime.getDayOfYear( date );
					pad = true;
					break;
				...
				case "c":
					if ( length <= 2 ) {
						...
						ret = ( date.getDay() - datetime.getFirstDayOfWeek( cldr ) + 8 ) % 7;
						pad = true;
						break;
					}
				...
				case "A":
					ret = Math.round( datetime.getMillisecondsInDay( date ) * Math.pow( 10, length - 3 ) );
					pad = true;
					break;
	 ...
	 * getFirstDayOfWeek
	 */
	datetime.getFirstDayOfWeek = function( cldr ) {
		// FIXME cldr
		return array.indexOf( datetime.weekDays, cldr.firstDay );
	};

	/**
	 * getMillisecondsInDay
	 */
	datetime.getMillisecondsInDay = function( date ) {
